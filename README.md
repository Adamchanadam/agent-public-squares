...
| 項目 | 目前狀態 |
|---|---|
| 公開版本 | `@adamchanadam/aps@0.2.21` |
| 最新 GitHub release | `v0.2.21` |
...
不是。`@adamchanadam/aps@0.2.21` 是可用於受控實際試行的前期測試版，不應承諾為不可中斷、不可出錯的生產級工具。
...
共同簡報會先定好：共同目標、參與者與 APS 名稱、每人角色、第一輪分工、不可做的事、驗收標準、第一個邀請對象和第一輪交接對象。文中的人名只作例子；實際參與者、角色和 APS 名稱由你提供或確認。這八項是最低欄位；如有截止日期、優先級、品牌語氣、審批點、合規限制、輸出格式、語言、預算或時間限制，可以叫 AI 加入。確認後，再邀請實際協作者，或發第一輪交接包。這一步可以避免每一邊的 AI 各自理解項目目標和分工，令後面的交接口徑不一致。
