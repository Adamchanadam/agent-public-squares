...
This table is the one-place APS Live product contract for first-screen behavior and local-AI handoff. Update it before changing generated APS Live HTML, `Check APS` / `check Drive` Live routing, local AI queue handoff, live-bridge user-facing behavior, APS skill guidance, or maintainer product-grade QC rules. It is a product contract, not proof that runtime already complies.
...
For the same APS collaboration state, sender-side and receiver-side pages may need different primary text:
...
- Existing roadmap: the historical product roadmap remains the reference for Reliable Peer Handoff, Project Context Index, and automation-out-of-scope decisions.
...
- Public docs must not claim APS Live is shipped as reliable cross-machine support until the Trystero transport passes two-machine verification and QC.
