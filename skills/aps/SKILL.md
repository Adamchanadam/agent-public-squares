...
3. **三項決定(三問安裝)**:逐項問,接著記入內部狀態 — APS 交換區完整路徑、APS 合作目錄名稱、你自己的用戶名稱(own agent_id)。APS 合作目錄必須用合作項目或任務命名,不得用人名、電腦名、AI 名稱或發起人名稱;例子應像 `community_open_day_parent_notice` 這類項目 / 任務代號,不要只給容易被理解成品牌或人名的短例。**不問對方是誰,也不問第一個交接包由誰先發**;起手方向由 CLI 自動推斷,只作預設提示,不用於授權。
...
14. **確認後的下一步**:
   ...
   - 若對方已是 confirmed peer 但尚未確認共同基準,第一張正式 APS packet 只能用 `shared_goal_and_roles` 作 topic。items 應明示「確認這份共同目標與分工、指出不一致或提出修訂」。不得把第一輪普通任務混入另一個 topic,也不得把普通任務 topic 當成共同基準確認。
   - 只有對方正式確認同一份 `shared_goal_and_roles` 後,才可轉第 6 節,用已確認基準加上該 peer 的第一輪任務建立普通交接草稿,再按發送前預檢與確認閘處理。
