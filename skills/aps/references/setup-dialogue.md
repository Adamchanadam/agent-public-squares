...
> 請確認三件事:一、這份共同目標與分工是否已夠安全開始;二、未定項是否已如實標出;三、之後可以用這份共同目標與分工作為這個 project 的目前有效基準。你確認後,我會再問你是否要本機保存、發給 confirmed peer 確認,或先邀請對方後再發確認包。

八項是 AI 的檢查框架,不是新手一開始必填表。起步時必須先有共同目標、自己的 agent id、第一個可做小步、明顯不可做事項、最小驗收方式;其他角色、邀請對象、第一輪交接對象或詳細驗收可先標為「未定」。若用戶有額外要求,可加上截止日期、優先級、參考檔、品牌語氣、審批點、合規限制、輸出格式、語言、預算或時間限制等欄位;但不要刪除共同目標、邊界與最小驗收方式等核心欄位。
