...
| Package manager | npm (`@adamchanadam/aps` published latest is 0.2.13 early-testing public release; local `package.json` is unpublished 0.2.14 candidate for S53 readable-summary + post-close-status + human-readable inbox-report UX root-fix; npm package name unchanged) | 2026-05-31 |
...
| `dev/release-notes/` | GitHub release note source files for tagged APS releases; `v0.2.14*` are local candidate notes, not yet published | before creating or reviewing a GitHub release |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;local version 0.2.14 candidate;published npm latest remains 0.2.13 as of S53 until npm publish is explicitly authorized and read back;repository / bugs / homepage URLs use the renamed `agent-public-squares` slug) | when adjusting package metadata, version, or publish channel |
...
| APS full audit | `dev/qc/2026-05-30-aps-full-audit-s50-governance-path-sync.md` is the latest scoped audit. The latest release-grade published audit remains `dev/qc/2026-05-30-aps-full-audit-0.2.13-release-check.md`. S53 two-folder evidence plus 0.2.14 candidate checks, including isolated inbox-report regression and the scoped 0.2.14 external-release-prep check, live in `dev/qc/evidence/2026-05-31-two-agent-uat/RESULTS.md`, `dev/SESSION_LOG.md`, and `dev/SESSION_HANDOFF.md`; no 0.2.14 public release audit / publish has happened yet. | when checking path-sync health, whether APS can be treated as fully verified before cross-workspace work, protocol promotion, public release, or new-agent introduction |
...
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.13 verified 2026-05-30 S48 publish;bin `aps`;fileCount 14;0.2.13 is the 人性化上手 early-testing public release;local 0.2.14 candidate is not published) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-31 |
