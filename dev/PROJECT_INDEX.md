...
|---|---|---|
| Agent Handoff Kit template version | 0.3.20 | 2026-05-31 |
...
| Package manager | npm (`@adamchanadam/aps` published latest is 0.2.14 readable packet UX root-fix; npm package name unchanged) | 2026-05-31 |
...
| `dev/release-notes/` | GitHub release note source files for tagged APS releases; `v0.2.14*` were used for the published 0.2.14 release | before creating or reviewing a GitHub release |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current published npm path is `npm install --save-dev @adamchanadam/aps@latest` then guided `npx aps init`;the published npm latest is 0.2.14 (S54, readable packet UX root-fix: readable notification / inbox summaries, successful post-close `status --packet-id`, and human-readable `inbox` receive reports before packet technical details); 0.2.13 (S48) shipped 人性化上手: three-question init + items explicit contract + `Agent_Public_Squares` default + forwardable starter-pack invite + joiner page; 0.2.12 (S44) was the naming & framing build; structural tokens unchanged | when iterating on install / setup flow, CLI packet flow, or before npm publish |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;local version 0.2.14 matches published npm latest 0.2.14;repository / bugs / homepage URLs use the renamed `agent-public-squares` slug) | when adjusting package metadata, version, or publish channel |
...
| `dev/qc/` | QC trigger vocabulary SSOT and per-tier checklists; `dev/qc/triggers.md` defines 🟢快檢 / 🟡外發前檢 / 🔴全面檢 plus the shared QC result closeout rule that forbids silently hiding failed / blocked / not-run items | when user invokes a QC trigger keyword, or when designing new QC checks |
...
| APS full audit | `dev/qc/2026-05-30-aps-full-audit-s50-governance-path-sync.md` is the latest scoped audit. The latest release-grade full audit remains `dev/qc/2026-05-30-aps-full-audit-0.2.13-release-check.md`; S54 0.2.14 was released after scoped external-release-prep plus post-publish readback. S53 two-folder evidence plus 0.2.14 candidate checks, including isolated inbox-report regression and the scoped 0.2.14 external-release-prep check, live in `dev/qc/evidence/2026-05-31-two-agent-uat/RESULTS.md`, `dev/SESSION_LOG.md`, and `dev/SESSION_HANDOFF.md`. | when checking path-sync health, whether APS can be treated as fully verified before cross-workspace work, protocol promotion, public release, or new-agent introduction |
...
| `dev/qc/triggers.md` | QC trigger vocabulary SSOT — 三 tier 定義 + QC 結果收口硬規則 + 嵌套規矩 + 反問規矩 + 既有 mechanism mapping | any QC trigger invocation; designing new QC checks; resolving QC scope ambiguity | local path | 2026-05-30 |
...
| `dev/qc/2026-05-30-aps-full-audit-s50-governance-path-sync.md` | Latest scoped APS audit for S50 folder rename / governance path sync — records path checks, Agent Handoff Kit doctor pass, active-governance old-path sweep, UAT sibling boundary, remaining SESSION_LOG N-rule warning, and strict full-audit gaps not run | before committing or relying on the S50 governance path sync | local path | 2026-05-30 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + early-testing boundary + neutral version wording (per npm `@latest`, currently published 0.2.14) guided CLI path with Agent Handoff Kit preflight (`npx --yes @adamchanadam/agent-handoff-kit@latest init`, then `npm install --save-dev @adamchanadam/aps@latest`, then `npx aps init`) + Project Peers + Sent Status + existing-project `npx aps upgrade` + setup values explained so readers do not copy placeholder paths; public route keeps internal `.md` planning / audit files out of the README quick path | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/agent-public-squares#readme | 2026-05-31 |

## External Sources

| Source | Role | Required before | Access method | Write-back rule | Last verified |
|---|---|---|---|---|---|
| APS Hub on Google Drive | runtime data store for cross-agent exchange — holds PROTOCOL.md, templates, lane data, packets, ack files | Phase 4 cross-machine handoff and any APS protocol or template change | per-machine local path (template; the maintainer's own machine sees it at `G:\我的雲端硬碟\Adam 工作目錄\AI_Projects\Agent_Public_Squares\`; other users' drive letters vary; offline-available required on every participating machine; older `AI_Public_Squares` folders remain compatible for existing installs) | this workspace owns no agent lane; writes only to `_hub/` for protocol / template / CHANGELOG updates; never writes to any `<project_slug>/from_<agent>/` or `_ack/*.ack.json` | 2026-05-30 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/agent-public-squares.git` (renamed from `ai-public-squares` 2026-05-29; HTTPS;auth via Windows Credential Manager,auto-pass;GitHub auto-redirects old web+git URLs but NOT Pages/Actions — do not reuse the old name) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-31 (`origin/main` and local HEAD both `cf367f1` after 0.2.14 release commit; tag `v0.2.14`→`cf367f1`; prior `v0.2.13`→`6059f45`, `v0.2.12`→`c9e8057`, and `v0.2.8`→`136042c` retained) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.14 verified 2026-05-31 S54 publish;bin `aps`;fileCount 14;0.2.14 is the readable packet UX root-fix release) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-31 |
| GitHub release `v0.2.14` (Latest) | public GitHub release record for Agent Public Squares 0.2.14 readable packet UX root-fix | before telling users 0.2.14 is released through GitHub | `https://github.com/Adamchanadam/agent-public-squares/releases/tag/v0.2.14` | create / update only with explicit release intent | 2026-05-31 (`isPrerelease=false`,`isDraft=false`; tag `v0.2.14`→`cf367f1`; body from `dev/release-notes/v0.2.14.github.md`; GitHub release list marks it Latest). Prior `v0.2.13`→`6059f45`, `v0.2.12`→`c9e8057`, and `v0.2.8`→`136042c` releases retained; 0.2.9–0.2.11 were npm-only with no tags |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/agent-public-squares/docs/index.html` (old `ai-public-squares` Pages URL now 404 — Pages project URLs do not redirect) | writes happen via `main` branch push only | 2026-05-31 (HTTP 200;entry page and joiner page both return 200 after 0.2.14 release push) |
...
| Codex `chrome-devtools` | `chrome-devtools-mcp@latest` | Chrome / DevTools browser inspection in future Codex sessions;usage statistics disabled | Codex global MCP config (`~/.codex/config.toml`);no project credential | 2026-05-26 | 2026-05-31 |
| Codex `context7` | `@upstash/context7-mcp` | current library / framework / SDK documentation lookup in future Codex sessions | Codex global MCP config (`~/.codex/config.toml`);no API key stored | 2026-05-26 | 2026-05-31 |
...
| Field | Value | Last verified |
|---|---|---|
| Expected project root | `C:\Users\adam\_claude_desktop\Agent_Public_Squares` | 2026-05-30 |
| Git root | same as above | 2026-05-30 |
| Branch / commit | `main`;0.2.14 release commit `cf367f1` pushed;tag `v0.2.14`→`cf367f1`;GitHub release `v0.2.14` is Latest;published npm latest is 0.2.14 | 2026-05-31 |
| Worktree or parallel workspace | stale local sibling `C:\Users\adam\_claude_desktop\AI_Public_Squares_UAT` was manually deleted by Adam after the formal S50 rename; this session verified it no longer exists. S53 self-UAT folders and isolated inbox-report regression live under ignored `dev/qc/evidence/2026-05-31-two-agent-uat/`; 0.2.14 source is now committed, pushed, tagged, released, and published. Any future UAT should be rebuilt as a clean workspace from npm `@latest` unless deliberately testing local source. Siblings `Demo_Agent_{Adam,Jay}_Public_Squares` are independent stores and must be verified in their own roots before relying on their governance health; GitHub Pages enabled at `https://adamchanadam.github.io/agent-public-squares/` | 2026-05-31 |
...
| Uncommitted change summary | S50-S53 governance path sync, closeout, self-UAT, and local CLI UX root-fix: Adam manually renamed the local project folder and Drive Hub, manually deleted stale local sibling `C:\Users\adam\_claude_desktop\AI_Public_Squares_UAT`, updated Agent Handoff Kit to v0.3.20, and asked to close out. Verified git root `C:\Users\adam\_claude_desktop\Agent_Public_Squares`; verified Drive Hub `G:\我的雲端硬碟\Adam 工作目錄\AI_Projects\Agent_Public_Squares\`; verified old Drive Hub path and UAT sibling no longer exist. Active governance files updated; historical plans / logs / QC evidence retain old path snapshots. Latest Kit migration `20260531T071022Z` merged `AGENTS.md`, updated PROJECT_INDEX metadata, and had no conflicts; closeout doctor passed 45 checks. SESSION_LOG N-rule advanced: S42/S41/S40 moved to archive batch 011 and S49-S43 collapsed to short index entries. S53 two-folder self-UAT under `dev/qc/evidence/2026-05-31-two-agent-uat/` passed core APS flow and found two CLI UX defects; local `bin/aps.js` now fixes shallow summary extraction, post-close status reporting, and `inbox` human-readable receive reports. A pre-existing unstaged edit had already updated the `context7` Last Verified cell to 2026-05-30 before S50 edits; S53 startup probe updated Codex MCP Last Verified cells to 2026-05-31. | 2026-05-31 |

## Change Hotspots

| Change type | Likely files | Required checks |
|---|---|---|
| APS protocol change | `G:\我的雲端硬碟\Adam 工作目錄\AI_Projects\Agent_Public_Squares\_hub\PROTOCOL.md` + `_hub/CHANGELOG.md` + both demo workspaces' `dev/rules/aps-bridge.md` + relevant section of `docs/plans/2026-05-20-agent-public-square-design.md` and `docs/plans/2026-05-20-aps-mvp-implementation.md` | inline-and-decouple consistency check across PROTOCOL ↔ Bridge Pack; doctor; template version-anchor comments updated; CHANGELOG entry with sign-off |
...
| `@adamchanadam/agent-handoff-kit` npm package | source of `AGENTS.md` managed core, governance file templates, doctor and upgrade commands | npm registry `https://www.npmjs.com/package/@adamchanadam/agent-handoff-kit`; use `npx --yes @adamchanadam/agent-handoff-kit@latest ...` for current doctor / upgrade checks | 2026-05-31 (doctor tool v0.3.20; tool / project record / npm latest 三向對齊) |
