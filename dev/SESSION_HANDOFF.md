...
- `bin/aps.js` + `skills/aps/**` + README + `docs/**`: `confirmed` — 0.2.14 shipped behaviour + teaching, published + pushed + live on Pages; README version hint corrected to 0.2.14; local 0.2.15 Project Context Index / Daily Index candidate is confirmed by `npm test`, browser snapshot evidence, `--help`, and pack dry-run. Agent Handoff Kit doctor is the only blocked governance check.
